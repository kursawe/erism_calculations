...
cavity = CompiledSubDomain('x[2]<=-0.05+tol',tol=tol)
stiff_layer = CompiledSubDomain('x[2]>-0.05-tol ', tol=tol)
materials = MeshFunction('size_t', mesh, 2,0)

stiff_layer.mark(materials,0)
cavity.mark(materials,1)
...
m_El=E_El/2/(1+nu)
l_El=E_El*nu/(1+nu)/(1-2*nu)

m_Sl=E_Sl/2/(1+nu)
l_Sl=E_Sl*nu/(1+nu)/(1-2*nu)

class G(UserExpression):
    def __init__(self, materials, m_0, m_1, **kwargs):
...
lamb = G(materials, l_Sl, l_El, degree=0)
mu= G(materials, m_Sl, m_El, degree=0)
...
def eps(v):
    return sym(grad(v))
def sigma(v):
    return lamb*tr(eps(v))*Identity(3) + 2.0*mu*eps(v)
def ppos(x):
    return (x+abs(x))/2.
...
plt.plot(v,y)
plt.plot(x,z)
plt.savefig('twoinone_stiff1.png')
...
a = sqrt(R*d)
F = 4/3.*(float(E_El)/(1-float(nu)**2))*a*d
p0 = 3*F/(2*pi*(a**2))

#corrected Hertz
L=8
chi=a/L
F_cor=4/3.*(float(E_El)/(1-float(nu)**2))*a*d*(1+1.133*chi+ 1.283*(chi**2)+0.769*(chi**3)+0.0975*(chi**4))
