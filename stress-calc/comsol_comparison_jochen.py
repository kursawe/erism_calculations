...
im4=Image.open('comsol_repeat_by_nils.tif')
im4 = np.rot90(im4,3)
stress_control=np.array(im4)
...
p.assign(-project(sigma(u)[2, 2], bcs = bcs, V=V2, solver_type="mumps"))
...
# Evalute stress at the figure resolution  
...
# plt.savefig('linear_output.png')

# Plot comparison to target file
fig, (ax0, ax1) = plt.subplots(1,2)

im1= ax0.pcolormesh(x,y,z, vmin = -35, vmax = 15, rasterized=True)
# im1= ax0.pcolormesh(x,y,z)
ax0.set_title('The calculated stress')
fig.colorbar(im1, ax=ax0)
ax1.set_title('The Comsol Stress')
im2= ax1.pcolormesh(x,y,im4, vmin = -35, vmax = 15,rasterized=True)
fig.colorbar(im2, ax=ax1)
ax0.set_aspect('equal', adjustable='box')
ax1.set_aspect('equal', adjustable='box')
plt.tight_layout()
plt.savefig('comparison_new_z_quadratic_components_vs_comsol.pdf')

max_diff = np.max(np.abs(z-im4)/im4)
mean_diff = np.mean(np.abs(z-im4)/im4)

print('max relative difference is')
print(max_diff)

print('mean relative difference is')
print(mean_diff)
