...
imarray=0.001*np.array(im)

im2=Image.open('Fig.1e.tif')
stress=np.array(im2)

Len=48.924
H=46.656

x_rev=np.linspace(Len,0, num=302)
y_rev=np.linspace(H,0, num=288)
x=np.linspace(0,Len, num=302)
...
#create mesh
N = 100
mesh=BoxMesh(Point(0,0,0), Point(Len,H,1),N,N,30)
...
x_plot=np.linspace(0, Len, num=N+1)
y_plot=np.linspace(0, H, num=N+1)
...
np.save('output2/output_100.npy', z)
print(z)
...
plt.savefig('output2/output_100.png')

file_results = XDMFFile("stress_calc_2.xdmf")
